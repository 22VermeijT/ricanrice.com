...
export default function OrderHero() {
  ...
  return (
    <section className="pt-36 pb-20 relative overflow-hidden">
      ...
      <FloralCluster
        variant="B"
        className="absolute top-6 right-8 opacity-35"
        glowColor="rgba(200,149,44,0.5)"
      />
      <FloralCluster
        variant="D"
      ...
      <FloralCluster
        variant="C"
        className="absolute top-1/2 right-2 -translate-y-1/2 opacity-20"
        glowColor="rgba(255,255,255,0.3)"
      />

      {/* Tropical leaves */}
      <div className="absolute -right-8 top-1/4 pointer-events-none opacity-12" style={{ transform: "rotate(165deg)" }}>
        <TropicalLeaf className="w-32 h-44" />
      </div>
      <div className="absolute -left-10 bottom-8 pointer-events-none opacity-10" style={{ transform: "rotate(-15deg)" }}>
      ...
      {[
        ...
        <div key={i} className="absolute pointer-events-none" style={pos}>
          <PrStar size={i % 2 === 0 ? 18 : 14} color="#C8952C" opacity={0.35} />
        </div>
      ))}

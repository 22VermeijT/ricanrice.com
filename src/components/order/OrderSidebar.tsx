...
// LUNCH REMOVED: Clock, PrStar, HibiscusFlower, motion kept for easy restoration of commented panels
...
export default function OrderSidebar() {
  ...
  // LUNCH REMOVED: schedule kept for easy restoration of lunch hours panel
  // const schedule = [
  ...
  return (
    <div className="space-y-6">
      {/* LUNCH REMOVED: lunch hours card — restore when lunch ordering returns
      <div className="bg-[#001435] text-white p-8 relative overflow-hidden">
        ...
        <motion.div className="absolute top-3 right-3 pointer-events-none opacity-15" animate={{ rotate: [-6, 6, -6] }} transition={{ duration: 7, repeat: Infinity, ease: "easeInOut" }} style={{ filter: "drop-shadow(0 2px 8px rgba(232,25,44,0.4))" }}>
          <HibiscusFlower className="w-10 h-10" />
        </motion.div>
        <div className="flex items-center gap-2 mb-6 relative z-10">
          <PrStar size={12} color="#E8192C" className="drop-glow-red" />
          <h3 className="text-xl font-bold" style={{ fontFamily: "var(--font-display)" }}>{t.orderPage.lunchHoursHeading}</h3>
        </div>
        ...
        <p className="mt-6 pt-6 border-t border-white/10 text-white/40 text-xs leading-relaxed relative z-10">{t.orderPage.lunchHoursNote}</p>
      </div>
      */}

      {/* LUNCH REMOVED: lunch pricing card — restore when lunch ordering returns
      <div className="bg-white border border-gray-200 p-8">
        <h3 className="text-xl font-bold text-[#001435] mb-5" style={{ fontFamily: "var(--font-display)" }}>{t.orderPage.pricingHeading}</h3>
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-2xl font-bold text-[#C8952C]" style={{ fontFamily: "var(--font-display)" }}>{t.orderForm.lunchPrice}</span>
        </div>
        <p className="text-xs text-[#6E6E73] border-t border-gray-100 pt-3 mb-4">{t.orderForm.freeDeliveryThreshold}.</p>
        {[t.ordering.deliveryLabel + ": " + t.ordering.deliverySub, t.ordering.pickupLabel + ": " + t.ordering.pickupSub].map((f) => (
